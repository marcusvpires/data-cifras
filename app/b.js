...
const db = {
    getAll: (callback = console.log) => {
        browser.storage.local.get(keyDB).then((result) => {
            let object = {}
            if (result[keyDB]) object = result[keyDB]
            callback(object)
        }, onError)
    },
    ...
    set: (object, callback = () => { }) => {
        browser.storage.local.set({ [keyDB]: object }).then(callback, onError)
    },
...
const updateMusicTable = (ID) => {
    db.get(ID, (playlist) => {
        console.log(ID, playlist)
        const musicas = playlist.musicas
        const first = createHTML("tr", [["class", "space"], ["style", "height: 20px;"]])
        const container = document.getElementById("playlist-tbody")
        container.innerText = ""
        container.appendChild(first)
        Object.entries(musicas).forEach(([ID, musica]) => {
            const tr = createHTML("tr", [["id", ID]])
            const name = musica.split(";")
            const title = createHTML("td", [["data-label", "Name"]])
            title.innerText = name[0]
            const autor = createHTML("td", [["data-label", "Autor"]])
            autor.innerText = name[1]
...
const openPlaylist = () => {
    ...
    db.get(ID, (playlist) => {
        document.getElementById("explorer-url-bar").style.display = "flex"
        document.getElementById("explorer-music-head").style.display = "table-row-group"
...
const constructorAddToList = (playlists) => {
    ...
    playlists.forEach(([ID, name, checked]) => {
        ...
        element.addEventListener("dbclick", openPlaylist)
...
const main = () => {
    ...
    document.getElementById("open").addEventListener("click", openPlaylist)
